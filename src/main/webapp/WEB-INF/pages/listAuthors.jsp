        ...
        <div id="divTable" style="position: absolute; top: 275px;left: 0px;width: 1000px">
            <table class="table">
                ...
                <c:forEach items="${pagedListHolder.pageList}" var="item">
                    <tr>
                        <td>${item.id}</td>
                        <td><a href="${pageContext.request.contextPath}/getAuthorById/${item.id}">${item.name}</a></td>
                        <td>${item.lastName}</td>
                        <td style="border: none"><a title="<spring:message code="app.admin.editAuthor"/>" class="editAuthor" data-id="${item.id}" onclick="location.href='${pageContext.request.contextPath}/admin/list/author?id=${item.id}'"><img src="${pageContext.request.contextPath}/resources/static/images/changeButton.png" alt="editing book" width="20px" height="20px"></a></td>
                    </tr>
                </c:forEach>
            </table>
